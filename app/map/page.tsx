...
export default function MapPage() {
    ...
    const copy = useMemo(
        () => ({
            pt: {
                ...
                today: "Hoje",
                boatsSection: "Barcos Praia ↔ Maio",
                flightsSection: "Voos Praia ↔ Maio",
                todayInMaio: "Hoje no Maio",
                todayHint: "Sugestão rápida com base no tempo e no mar.",
                morning: "Manhã",
                midday: "Meio-dia",
            ...
            en: {
                ...
                today: "Today",
                boatsSection: "Boats Praia ↔ Maio",
                flightsSection: "Flights Praia ↔ Maio",
                todayInMaio: "Today in Maio",
                todayHint: "Quick plan based on sea and wind.",
                morning: "Morning",
                midday: "Midday",
                lateDay: "Late day",
    ...
    useEffect(() => {
        ...
        map.on("load", () => {
            ...
            map.on("click", "beaches-fill", (e) => {
                ...
                const matchedPlace =
                    storyPlacesRef.current.find((p: any) => p?.osm_id && p.osm_id === osmId) ||
                    storyPlacesRef.current.find((p: any) => {
                        const candidate =
          ...
          ========================= */
            map.on("click", "settlements-points", (e) => {
                ...
                const matchedPlace = storyPlacesRef.current.find((p: any) => {
                    const candidate =
                        typeof p?.name === "string"
                            ? p.name
                            : p?.name?.pt || p?.name?.en;
                    if (!candidate || !f.properties?.name) return false;
                    return normalizeLabel(candidate) === normalizeLabel(f.properties.name);
                });
                ...
                setSelectedSettlement({
                    ...
                    description,
                    image: f.properties?.image || matchedPlace?.image_url,
                    coordinates: coords,
                });
            ...
            map.on("click", "protected-areas-fill", (e) => {
                ...
                const protectedMatch = storyPlacesRef.current.find((p: any) => {
                    const candidate =
                        typeof p?.name === "string"
                            ? p.name
                            : p?.name?.pt || p?.name?.en;
                    if (!candidate) return false;
                    return normalizeLabel(candidate) === normalizeLabel(protectedName);
                });
                ...
                setSelectedProtectedArea({
                    ...
                    category: f.properties?.designation || "Área protegida",
                    description,
                    image: protectedMatch?.image_url,
                    coordinates: [e.lngLat.lng, e.lngLat.lat],
                });
    ...
    const getDefaultImageForPlace = (place: any) => {
        const name = getPlaceLabel(place?.name);
        const location = getPlaceLabel(place?.location);
        const haystack = `${name} ${location}`.toLowerCase();

        const defaults: Array<{ keywords: string[]; image: string }> = [
            {
                keywords: ["parque natural do norte", "norte", "norte da ilha", "ilha do maio natural park"],
                image: "/places/parquenorte.jpg",
            },
            {
                keywords: ["barreiro", "figueira", "parque natural de barreiro e figueira", "ribeira of lagoa"],
                image: "/places/capelafigueira.png",
            },
            { keywords: ["praia do morro", "borro beach", "morro", "morrinho"], image: "/places/praiadomorro.png" },
            {
                keywords: ["vila", "porto ingl", "porto inglês", "porto ingles", "cidade"],
                image: "/places/portopraia.png",
            },
            { keywords: ["calheta"], image: "/places/capelacalheta.png" },
            { keywords: ["lagoa"], image: "/places/lagoa.png" },
            { keywords: ["santana"], image: "/places/praiasantana.png" },
        ];

        const match = defaults.find((entry) =>
            entry.keywords.some((keyword) => haystack.includes(keyword))
        );
        return match?.image || "/image.png";
    };
    ...
    const buildChapters = (places: any[], language: "pt" | "en") => {
        ...
        return places
            ...
            .slice()
            .sort((a, b) => {
                if (a.id === "cidade-porto-ingles") return -1;
                if (b.id === "cidade-porto-ingles") return 1;
    ...
    const todayPlan = useMemo(() => {
        ...
        const pickTop = (slot: "morning" | "midday" | "late", used: Set<string>) => {
            const ranked = places
                .filter((p) => !used.has(p.id))
                .map((p) => ({ ...p, score: scorePlace(p, slot) }))
        ...
        const morning = pickTop("morning", used);
        const midday = pickTop("midday", used);
        const late = pickTop("late", used);
    ...
    return (
        <>
            ...
            <div className="relative z-10 max-w-6xl mx-auto px-6 pt-18 pb-6 flex flex-col gap-6">
                ...
                <div className="flex flex-col gap-4 overflow-hidden">
                    <div className="order-2 space-y-4">
                        ...
                        <div className="mt-4 rounded-2xl border border-border bg-background p-5 shadow-sm">
                            ...
                            <div className="hidden mt-3 text-xs text-muted-foreground">
                                {todayConditions?.label ||
                                    (lang === "pt" ? "A carregar condições..." : "Loading conditions...")}
                            </div>
                            <div className="mt-3 grid gap-3 sm:grid-cols-3">
                                {todayPlan.length === 0 && (
                                    <div className="text-xs text-muted-foreground">
                                        {lang === "pt" ? "A carregar sugestões..." : "Loading ideas..."}
                                    </div>
                                )}
                                {todayPlan.map((item) => (
                                    <a
                                        ...
                                        className="group rounded-2xl border border-border bg-background/80 p-3 text-left shadow-sm transition hover:shadow-md"
                                    >
                        ...
                        <div className="mt-4 grid gap-3 sm:grid-cols-2">
                            <div className="rounded-2xl border border-border bg-background p-5 shadow-sm">
                                ...
                                <div className="mt-4 space-y-3 text-sm text-muted-foreground">
                                    {boatsLoading && (
                                        <div>{lang === "pt" ? "A carregar..." : "Loading..."}</div>
                                    )}
                                                ...
                                                .map((group, idx) => (
                                                    <div key={`boat-day-${idx}`} className="space-y-3">
                                                        ...
                                                        <div className="grid gap-3 sm:grid-cols-2">
                                                            {[group.items.find((i: any) => i.to === "Maio"), group.items.find((i: any) => i.to === "Santiago")]
                                                                .filter(Boolean)
                                                                .map((item: any, j: number) => (
                                                                    <div
                                                                        key={`boat-item-${idx}-${j}`}
                                                                        className="rounded-2xl border border-border bg-background/80 p-3 shadow-sm"
                                                                    >
                                            ...
                                            const renderCard = (item: any, from: "RAI" | "MMO", to: "RAI" | "MMO") => {
                                                ...
                                                return (
                                                    <button
                                                        ...
                                                        className="rounded-2xl border border-border bg-background/80 p-3 text-left shadow-sm transition active:scale-[0.98]"
                                                    >
                    ...
                    >
                        ...
                        >
                            ...
                            <div className="absolute top-4 left-3 right-16 sm:right-20 z-30 flex items-center gap-2">
                                <button
                                    type="button"
                        ...
                        {coordCopied
                            ...
                            >
                                <Drawer.Portal>
                                    ...
                                    <Drawer.Content className="fixed inset-x-0 bottom-0 z-[70] rounded-t-3xl border border-border bg-background p-4 pb-8 shadow-xl">
                            ...
                            <Drawer.Root open={exploreOpen} onOpenChange={setExploreOpen}>
                                <Drawer.Portal>
                                    ...
                                    <Drawer.Content
                                        className="fixed inset-x-0 bottom-0 z-[60] rounded-t-3xl border border-border bg-background p-5 pb-10 shadow-xl"
                                        style={
                                            {
                                                "--initial-transform": "calc(100% + 12px)",
                                                paddingBottom:
