...
const ANNUAL_2025_VALUE = 107960558;
...
async function run() {
  ...
  const filtered = baseData.filter((row) => Number(row?.MES) !== 12);
  const updatedData = [
    ...filtered,
    {
      MES: 12,
      VALOR_PAGO: ANNUAL_2025_VALUE,
      SIGLA: MUNICIPIO,
    },
    ...
    { key: KEY },
    {
      $set: {
        ...
        meta: {
          ...
          note: "2025 atualizado com fecho anual consolidado (MES=12).",
        },
        updatedAt: new Date(),
      },
      $setOnInsert: {
  ...
  await client.close();
  console.log(
    `Transferências atualizadas: ${KEY} (MES=12, VALOR_PAGO=${ANNUAL_2025_VALUE})`
  );
}
