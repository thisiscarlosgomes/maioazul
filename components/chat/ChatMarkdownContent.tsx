...
export default function ChatMarkdownContent({ content }: ChatMarkdownContentProps) {
  return (
    <Streamdown
      ...
      components={{
        ul: ({ children }) => <ul className="list-outside list-disc whitespace-normal pl-5">{children}</ul>,
        ol: ({ children }) => <ol className="list-outside list-decimal whitespace-normal pl-5">{children}</ol>,
        li: ({ children }) => <li className="my-0 leading-6">{children}</li>,
        p: ({ children }) => <p className="leading-6 my-0">{children}</p>,
        a: (props) => (
          <a
            {...props}
