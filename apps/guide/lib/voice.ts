...
const SPEED_KEY = "maio-voice-speed";
const VOICE_SPEED_DEFAULT = 1;
const VOICE_SPEED_MIN = 1;
const VOICE_SPEED_MAX = 2;
export const VOICE_SPEED_OPTIONS = [1, 1.25, 1.5, 1.75, 2] as const;
...
const clampVoiceSpeed = (speed: number) => {
  if (!Number.isFinite(speed)) return VOICE_SPEED_DEFAULT;
  return Math.min(VOICE_SPEED_MAX, Math.max(VOICE_SPEED_MIN, speed));
};

const readStoredSpeed = () => {
  if (typeof window === "undefined") return VOICE_SPEED_DEFAULT;
  const raw = window.localStorage.getItem(SPEED_KEY);
  if (!raw) return VOICE_SPEED_DEFAULT;
...
const getRuntime = (): VoiceRuntime | null => {
  ...
  const hydrate = () => {
    try {
      ...
      audio.playbackRate = runtime.speed;
...
export const getVoiceSpeed = () => {
  const runtime = getRuntime();
  return runtime?.speed ?? VOICE_SPEED_DEFAULT;
};

export const setVoiceSpeed = (speed: number) => {
  const runtime = getRuntime();
  if (!runtime) return;
  const next = clampVoiceSpeed(speed);
  runtime.speed = next;
  if (typeof window !== "undefined") {
    window.localStorage.setItem(SPEED_KEY, String(next));
  }
  if (runtime.audio) {
    runtime.audio.playbackRate = next;
  }
  notify(runtime);
};
...
export const useVoiceSpeed = () => {
  ...
  useEffect(() => subscribeVoice(() => setSpeed(getVoiceSpeed())), []);
  return speed;
};
